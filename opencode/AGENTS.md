...
- **提交（commit）时只能提交本会话中修改过的文件**，不得提交会话开始前就已被修改或未跟踪的旧文件。提交前应先用 `git diff --name-only` 或 `git status` 确认只包含本会话的修改，使用 `git add` 明确指定要提交的文件列表，禁止 `git add -A` 或 `git add .` 全量暂存。
